...
- 相位自动校准锁定后，KEY5/KEY6 分别增加/减小相位补偿，每次微调 1/16 个 AD 采样点。
...
AD2 回环信号的正向过零位置：反馈落后时增加一个采样点的 DDS 相位提前量，反馈
超前时减少一个采样点；连续三次相位误差不超过 ±1 个采样点后置位内部
`phase_cal_locked`。得到的补偿按采样点保存，并自动随测得频率换算为 DDS 相位；
正交和二倍频模式沿用这份补偿，不在这些模式中更新校准环。

自动校准锁定后会冻结整采样点粗调，避免反馈环抵消人工操作。人工微调量
`manual_phase_trim_q4` 采用 Q4 采样点单位：KEY5 加 1，KEY6 减 1，对应每次
1/16 个采样点，范围为 ±32 个采样点。微调先按当前实测周期换算为 32 位 DDS
相位字，再参与直通、正交和二倍频相位计算，因此频率改变后无需重新换算按键步长。
在 12.5 MHz 采样率下，100 kHz 直通模式每次约为 0.18°，二倍频模式约为
0.36°；1 kHz 直通模式每次约为 0.0018°。
...
- KEY5：仅在有线模式且相位校准锁定后有效，每按一次增加 1/16 采样点相位补偿。
- KEY6：仅在有线模式且相位校准锁定后有效，每按一次减少 1/16 采样点相位补偿。
...
测频与 DDS 锁定、回环相位校准、锁定前微调屏蔽、锁定后 Q4 相位加减、四路独立
...
- 三级采样回环模型下，相位环在 622 个采样内锁定到 2 个采样点补偿，残差为 1 个采样点，AD1/AD2 同相相关量为 1.0005。
- KEY5/KEY6 的锁前屏蔽、锁后 ±1/16 采样点调整、锁定状态保持及无线模式屏蔽均通过。
- 1 kHz/100 kHz 测频与 DDS 锁定、四路独立 CLK、两路 AD OE、双 DAC 等幅复制、四档幅度、三个图形、有线按键控制、无线按键屏蔽、LED1 状态和无线安全输出全部通过。
- 100 kHz 校准后正交反馈与输入的归一化相关量为 0.0626，二倍频测试窗口内检测到 10 次过零；1 kHz 测得周期为 12500 个采样点。
